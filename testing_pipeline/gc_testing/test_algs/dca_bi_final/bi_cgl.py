...
sys.path.append(root_dir+'/test_algs/dca_bi_final/')
...
def run_main(args):
    ...
    GroundTruth = args['alg_loader'].dataset.dependencies['dep1'].to_numpy()
    ...
    best_val = 10000000
    optim = Optim.Optim(
        model.parameters(), args['optim'], args['learning_rate'], args['clip'], start_decay_at = args['start_decay_at'], weight_decay = args['weight_decay'])
    ...
    weight_norm = np.zeros((args['train_epochs'], num_weight_plot))
    auc1_best = 0.1
